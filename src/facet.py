    ...
    for huc_dir in HUC_LST:
        ...
        if not str_dem_proj.is_file():
            str_dem_path_proj = funcs.reproject_grid_layer(
                str_dem, PARAMS['crs'], str_dem_proj,
                PARAMS['resample resolution'], logger)
        else:
            logger.info(f'{str_dem_proj} already exists. DEM re-projection and resampling skipped.')
        ...
        if PARAMS['pre-condition dem & fast-breach']:
            ...
            dem_merge = funcs.pre_breach_DEM_conditioning(
                huc_dir, HUCID, str_dem_proj,
                str_nhdhr_huc10, PARAMS['census roads prj'], PARAMS['census rails prj'],
                huc10_mask_proj, logger)
        ...
        funcs.preprocess_dem(
            huc_dir, str_nhdhr_huc10, PARAMS['crs'],
            PARAMS['wt_grid'], PARAMS['taudem'], PARAMS['physio prj'],
            HUCID, str_breached_dem_path, PARAMS['taudem cores'],
            logger)
        ...
        df_coords, streamlines_crs = funcs.get_stream_coords_from_features(
            str_net_path, cell_size, PARAMS['reach_id'],
            PARAMS['order_id'], logger)
        ...
        # Floodplain:
        funcs.write_xns_shp(df_coords, streamlines_crs, str_fpxns_path, True, int(30), logger)

        # << INTERPOLATE ELEVATION ALONG Xns >>
        df_xn_elev = funcs.read_xns_shp_and_get_dem_window(str_chxns_path, str_dem_proj, logger)

        # Calculate channel metrics and write bank point shapefile
        funcs.chanmetrics_bankpts(
            df_xn_elev, str_chxns_path, str_dem_proj,
            str_bankpts_path, PARAMS['parm_ivert'], PARAMS['xnptdist'],
            PARAMS['parm_ratiothresh'], PARAMS['parm_slpthresh'], logger,
            PARAMS['crs'])


        # ========== << BANK PIXELS AND WIDTH FROM CURVATURE >> ==========
        funcs.bankpixels_from_curvature_window(
            df_coords, str_dem_proj, str_bankpixels_path,
            cell_size, PARAMS['use_wavelet_curvature_method'], logger)

        funcs.channel_width_from_bank_pixels(
            df_coords, str_net_path, str_bankpixels_path,
            PARAMS['reach_id'], PARAMS['i_step'], PARAMS['max_buff'],
            str_chanmet_segs, logger)

        # ========== << DELINEATE FIM >> ==========
        funcs.fim_hand_poly(
            str_hand_path, str_sheds_path, PARAMS['reach_id'],
        ...
        funcs.hand_analysis_chsegs(
            str_hand_path, str_chanmet_segs, str_raster_net_path,
            str_fim_path, str_dem_proj, logger)

        # capture core run time for FACET algorithms
        end_core_facet_time = round((timer() - st_core_facet_time)/60.0, 2)
        logger.info(f'Core FACET algorithms completed. Time elapsed: {end_core_facet_time} mins')
        ...
        if PARAMS['post process']:
            dbf_tuple = post_process.flag_FACET_outputs(
                str_nhdhr_huc10, PARAMS['waterbody prj'], PARAMS['stream buffer'], huc10_mask_proj,
                str_chanmet_segs, str_bankpts_path, str_fpxns_path,
                post_process_dir, logger)
