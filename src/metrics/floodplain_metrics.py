...
def fp_metrics_chsegs(flood_extent_layer, ch_width_id, channel_segs, xn_type, logger):
    ...
    Args:
        flood_extent_layer: path to the flood inundation raster
        ch_width_id: name of the field in the channel segment layer containing pre-calculated channel width values
        channel_segs: path to the segmented streamline layer containing pre-calculated channel metrics
    ...
    gdf_segs = gpd.read_file(channel_segs)
    ...
    with rasterio.open(flood_extent_layer) as ds_fim:
        ...
        for tpl in gdf_segs.itertuples():
            ...
            try:
                ...
                p_xnlength, p_fitlength = xn_type[str(tpl.order)].values()
                ...
                fp_width = fp_width - getattr(tpl, ch_width_id)
    ...
    gdf_segs.to_file(channel_segs)  


def read_fp_xns_shp_and_get_1D_fp_metrics(
    floodplain_xns, flood_extent_layer, dem, logger
):
    """
    ...
    with rasterio.open(dem) as ds_dem:
        with rasterio.open(flood_extent_layer) as ds_fp:
            ...
            for linkno, gdf in gp_xns:
                ...
                for i, tpl in enumerate(gdf.itertuples()):
                    try:
                        ...
                        print(f"Error with itertuple: {e}")
...
def hand_method(
    hand, channel_segs, network_poly, network_rast, flood_extent_layer, dem, xn_type, logger
):
    """
    ...
    Args:
        hand: Path to the HAND grid .tif
        channel_segs: Path to the output of the channel_width_from_bank_pixels() func
        network_rast: Path to the .tif file where stream reaches correspond to linkno values
        flood_extent_layer: Path to the floodplain grid .tif
        dem:
        logger: Logger instance for messaging

    Returns: Writes out additional attributes to the file in channel_segs
    """

    utils.rasterize(network_poly, hand, network_rast, "LINKNO")
    ...
    with rasterio.open(hand) as ds_hand:
        ...
        with rasterio.open(network_rast) as ds_src:
...
def derive(
    floodplain_xns, flood_extent_layer, dem, 
